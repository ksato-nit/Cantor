<?php
$input = file_get_contents("mumford_projective.cpp");
$rows = explode("\n", $input);
$output = "";

foreach($rows as $ind => $row){
    $pattern = "/.*mpz_add\([a-zA-Z0-9].*\, [a-zA-Z0-9].*\, [a-zA-Z0-9].*\).*/";
    $match = preg_match($pattern, $row, $matches);
    if($match != false){
        $start = strpos($row, "mpz_add");
        ...
        $row1 = "mpz_add(" . $op1 . ".re, " . $op2 . ".re, " . $op3 . ".re);";
        $row2 = "mpz_add(" . $op1 . ".im, " . $op2 . ".im, " . $op3 . ".im);";
        $output .= $row1 . "\n" . $row2 . "\n";
    }else{
        $output .= $row . "\n";
    }
